...
@endsection
@section('meta')
    <meta property="og:title" content="{{$digitalstore->name}}" />
  <meta property="og:url" content="{{Request::fullUrl()}}" />
  <meta property="og:image" content="{{URL::to($digitalstore->image)}}" />
  <meta property="og:description" content="$digitalstore->description" />
    ...
    <div class="container">
        <div class="row">
            <div class="col-lg-8 col-xs-12 col-sm-12">
                <div class="sv-video">
                	<img src="{{ asset($digitalstore->image) }}" class="img-fluid"style="width:100%;height:100%;"  width="100%" height="100%" >
                ...
                <h1><a href="#">{{$digitalstore->name}}</a></h1>
                ...
                <div class="author">
                    <div class="author-head">
                        <a href="{{route('home.user.details',[$digitalstore->user->id,$digitalstore->user->slug])}}"><img src="{{ asset($digitalstore->user->profile_photo_path) }}" alt="" class="sv-avatar"></a>
                        <div class="sv-name">
                            <div><a href="#">{{$digitalstore->user->name}}</a></div>
                            <div class="c-sub hidden-xs">
                                <div class="c-f">
                                    @if( Route::has('login'))
                                        @auth
                                        @php
                                        $subscrive = App\Models\UserSubscrive::where('user_id',Auth::user()->id)->where('subscrive_user_id',$digitalstore->user->id)->count();
                                        @endphp
                                        @if($subscrive > 0)
                                        <a href="{{route('user.un.subscribe',[Auth::user()->id,$digitalstore->user->id])}}" class="sub-a">Subscribed</a>
                                        @else
                                        @if($digitalstore->user->id == Auth::user()->id)
                                        Subscribe
                                        @else
                                        <a href="{{route('user.subscribe',$digitalstore->user->id)}}" class="sub-a">Subscribe</a>
                                        @endif
                                        @endif
                                ...
                                <div class="c-s">
                                    @php
                                        $subscrive = App\Models\UserSubscrive::where('subscrive_user_id',$digitalstore->user->id)->count();
                                    @endphp
                                    {{$subscrive}}
                                </div>
                    ...
                    <div class="custom-tabs">
                        <div class="tabs-panel">
                            ...
                            <div class="acide-panel hidden-xs">
                                 <a href="#"><i class="cv cvicon-cv-watch-later" data-toggle="tooltip" data-placement="top" title="Watch Later"></i></a>
                                 @php 
                                    $like = App\Models\likeDigitalstore::where('user_id',Auth::user()->id)->where('digital_store_id',$digitalstore->id)->count();
                                 @endphp
                                 @if($like > 0)
                                 <a href="{{route('unlike.digitalstore',[$digitalstore->id,$digitalstore->user->id])}}"><i class="cv cvicon-cv-liked" style="color: red!important" data-toggle="tooltip" data-placement="top" title="Liked"></i></a>
                                 @else
                                
                                 <a href="{{route('like.digitalstore',[$digitalstore->id,$digitalstore->user->id])}}"><i class="cv cvicon-cv-liked" data-toggle="tooltip" data-placement="top" title="Like"></i></a>
                        ...
                        <div class="tabs-content">
                            ...
                            <div class="tab-1">
                                <div>
                                    ...
                                    <h4>Category :</h4>
                                    <p>{{$digitalstore->category->category_name}}</p>
                                    <h4>Sub-Category :</h4>
                                    <p>
                                    {{$digitalstore->subcategory->templete_sub_category_name}}
                                    </p>
                                    <h4>About :</h4>
                                    <p>{!!$digitalstore->description!!}</p>

                                    <h4>Tags :</h4>
                                    <p class="sv-tags">
                                        @foreach($digitalstore->tags as $tag)
                                        <span><a href="{{route('home.tag.digitalstore.details',$tag->tag_name)}}">{{$tag->tag_name}}</a></span>
                                        @endforeach
                                    </p>
                                    ...
                                    <div class="row date-lic">
                                        <div class="col-xs-6">
                                            <h4>Release Date:</h4>
                                            <p>{{$digitalstore->created_at->diffForHumans()}}</p>
                                        </div>
                            ...
                            <div class="tab-4">
                                ...
                                <div class="block-list">
                                	@foreach($digitalstoreTemplates as $digitalstoreTemplate)
                                    <div>
                                        <a href="{{route('home.template.details',[$digitalstoreTemplate->template->id,$digitalstoreTemplate->template->slug])}}">
                                        <span class="name">{{$digitalstoreTemplate->template->title}}</span></a>
                                        <a href="{{route('template.download',$digitalstoreTemplate->template->id)}}">
                    ...
                    <div class="single-v-footer">
                        <div class="single-v-footer-switch">
                            <a href="#" class="active" data-toggle=".similar-v">
                                ...
                                <span>Similar Digitalstore</span>
                            </a>
                        ...
                        <div class="similar-v single-video video-mobile-02">
                            <div class="row">
                                ...
                                @forelse(App\Models\DigitalStore::orWhere('name', 'LIKE' ,$digitalstore->name)->orWhere('user_id',$digitalstore->user_id)->where('id', '!=' ,$digitalstore->id)->get()->take(10) as $insData)
                                    @if($insData->id === $digitalstore->id)
                                    @else
                                    <div class="col-lg-3 col-sm-6 col-xs-12">
                                        <div class="h-video row">
                                            <div class="col-sm-12 col-xs-6">
                                                <div class="v-img">
                                                    <a href="{{route('home.digitalstore.details',[$insData->id,$insData->slug])}}"><img src="{{ asset($insData->image) }}" alt=""></a>
                                                    
                                                </div>
                                            </div>
                                            <div class="col-sm-12 col-xs-6">
                                                <div class="v-desc">
                                                    <a href="{{route('home.digitalstore.details',[$insData->id,$insData->slug])}}">{{$insData->name}}</a>
                                                </div>
                                                <div class="v-views">
                                                    630,347 views
                                                </div>
                        ...
                        <div class="comments">
                            <div class="reply-comment">
                                ...
                                <div class="rc-comment">
                                    @if( Route::has('login'))
                                        @auth
                                            <form action="{{route('digitalstore.comment.store')}}" method="post">
                                                @csrf
                                                <input type="hidden" name="digitalstore_id" value="{{$digitalstore->id}}">
            ...
            <div class="col-lg-4 col-xs-12 col-sm-12 hidden-xs">
                ...
                <div class="caption">
                    <div class="left">
                        <a href="#">Recomended Digital Store</a>
                    </div>
