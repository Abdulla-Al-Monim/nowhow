                            ...
                            <div class="c-sub hidden-xs">
                                <div class="c-f">
                                   @if( Route::has('login'))
                                        @auth
                                        @php
                                        $subscrive = App\Models\UserSubscrive::where('user_id',Auth::user()->id)->where('subscrive_user_id',$video->user->id)->count();
                                        @endphp
                                        @if($subscrive > 0)
                                        <a href="{{route('user.un.subscribe',[Auth::user()->id,$video->user->id])}}" class="sub-a">Subscribed</a>
                                        @else
                                        @if($video->user->id == Auth::user()->id)
                                        Subscribe
                                        @else
                                        <a href="{{route('user.subscribe',$video->user->id)}}" class="sub-a">Subscribe</a>
                                        @endif
                                        @endif
                                ...
                                <div class="c-s">
                                    @php
                                        $subscrive = App\Models\UserSubscrive::where('subscrive_user_id',$video->user->id)->count();
                                    @endphp
                                    {{$subscrive}}
                                </div>
                    ...
                    <div class="custom-tabs">
                        <div class="tabs-panel">
                            ...
                            <div class="acide-panel hidden-xs">
                                 <a href="#"><i class="cv cvicon-cv-watch-later" data-toggle="tooltip" data-placement="top" title="Watch Later"></i></a>
                                 @php 
                                    $like = App\Models\likeVideo::where('user_id',Auth::user()->id)->where('video_id',$video->id)->count();
                                 @endphp
                                 @if($like > 0)
                                 <a href="{{route('unlike.video',[$video->id,$video->user->id])}}"><i class="cv cvicon-cv-liked" style="color: red!important" data-toggle="tooltip" data-placement="top" title="Liked"></i></a>
                                 @else
                                
                                 <a href="{{route('like.video',[$video->id,$video->user->id])}}"><i class="cv cvicon-cv-liked" data-toggle="tooltip" data-placement="top" title="Like"></i></a>
