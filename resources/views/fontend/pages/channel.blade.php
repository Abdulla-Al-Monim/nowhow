...
@section('meta')
  ...
  <meta property="og:image" content="{{URL::to($channel->image)}}" />
                            ...
                            <div class="c-sub hidden-xs">
                                <div class="c-f">
                                    @if( Route::has('login'))
                                        @auth
                                        @php
                                        $subscrive = App\Models\UserSubscrive::where('user_id',Auth::user()->id)->where('subscrive_user_id',$channel->user->id)->count();
                                        @endphp
                                        @if($subscrive > 0)
                                        <a href="{{route('user.un.subscribe',[Auth::user()->id,$channel->user->id])}}" class="sub-a">Subscribed</a>
                                        ...
                                        Subscribe
                                        @else
                                        <a href="{{route('user.subscribe',$channel->user->id)}}" class="sub-a">Subscribe</a>
                                        @endif
                                        @endif
                                ...
                                <div class="c-s">
                                    @php
                                        $subscrive = App\Models\UserSubscrive::where('subscrive_user_id',$channel->user->id)->count();
                                    @endphp
                                    {{$subscrive}}
                                </div>
                ...
                <div class="info">
                    <div class="custom-tabs">
                        <div class="tabs-panel">
                            ...
                            <div class="acide-panel hidden-xs">
                                 <a href="#"><i class="cv cvicon-cv-watch-later" data-toggle="tooltip" data-placement="top" title="Watch Later"></i></a>
                                 @php 
                                    $like = App\Models\LikeChannel::where('user_id',Auth::user()->id)->where('channel_id',$channel->id)->count();
                                 @endphp
                                 @if($like > 0)
                                 <a href="{{route('unlike.channel',[$channel->id,$channel->user->id])}}"><i class="cv cvicon-cv-liked" style="color: red!important" data-toggle="tooltip" data-placement="top" title="Liked"></i></a>
                                 @else
                                
                                 <a href="{{route('like.channel',[$channel->id,$channel->user->id])}}"><i class="cv cvicon-cv-liked" data-toggle="tooltip" data-placement="top" title="Like"></i></a>
                        ...
                        <div class="tabs-content">
                            ...
                            <div class="tab-1">
                                <div>
                                    ...
                                    <h4>Category :</h4>
                                    <p>{{$channel->category->category_name}}</p>
                                    <h4>Sub Category :</h4>
                                    <p>{{$channel->subCategory->video_sub_category_name}}</p>
                                    ...
                                    <p class="sv-tags">
                                        ...
                                        <span><a href="{{route('home.tag.channels.details',$tag->tag_name)}}">{{$tag->tag_name}}</a></span>
                                        @endforeach
                                    </p>
                                    ...
                                    <div class="row date-lic">
                                        <div class="col-xs-6">
                                            <h4>Release Date:</h4>
                                            <p>{{$channel->created_at->diffForHumans()}}</p>
                                        </div>
                            ...
                            <div class="tab-4">
                                ...
                                <div class="block-list">
                                    ...
                                    <div>
                                        <a href="{{route('home.video.details',[$channelVideo->video->id,$channelVideo->video->slug])}}">
                                        <span class="name">{{$channelVideo->video->title}}</span></a>
                                        <a href="{{route('video.download',$channelVideo->video->id)}}">
                    ...
                    <div class="single-v-footer">
                        ...
                        <div class="similar-v single-video video-mobile-02">
                            <div class="row">
                                @forelse(App\Models\Channel::orWhere('name', 'LIKE' ,$channel->name)->orWhere('user_id',$channel->user_id)->where('id', '!=' ,$channel->id)->get()->take(10) as $insData)
                                    @if($insData->id === $channel->id)
                                    @else
                                    <div class="col-lg-3 col-sm-6 col-xs-12">
                                        <div class="h-video row">
                                            <div class="col-sm-12 col-xs-6">
                                                <div class="v-img">
                                                    <a href="{{route('home.channel.details',[$insData->id,$insData->slug])}}"><img src="{{ asset($insData->image) }}" alt=""></a>
                                                    
                                                </div>
                                            </div>
                                            <div class="col-sm-12 col-xs-6">
                                                <div class="v-desc">
                                                    <a href="{{route('home.channel.details',[$insData->id,$insData->slug])}}">{{$insData->name}}</a>
                                                </div>
                                                <div class="v-views">
                                                    630,347 views
                                                </div>
                        ...
                        <div class="comments">
                            <div class="reply-comment">
                                ...
                                <div class="rc-comment">
                                    <form action="{{route('channel.comment.store')}}" method="post" >
                                        @csrf
