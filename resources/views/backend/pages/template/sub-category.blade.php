...
@section('user-sub-temp-cat-active')
    active
@endsection
                                            ...
                                            <div class="col-md-12 mb-4">
                                                ...
                                                <select class="form-control  basic" id="id">
                                                    <option >Category</option>
    													    @foreach(App\Models\UserTempleteCat::get() as $cat)
													<option value="{{$cat->id}}">{{$cat->category_name}}</option>
													@endforeach
											</select>
                                            </div>
                                        </div>
                                        <button class="btn btn-primary submit-fn mt-2" id="addBtn" onclick="addData()" type="submit">Add Sub-category</button>
                                    </form>
	...
	function clearData(){
     ...
     function allData() {
        $.ajax({
          type: "GET",
          dataType: "json",
          url: "/admin/template/sub-category/all",
          success:function (response) {
            var data = "";
            $.each(response, function(key,value){
              data = data + "<tr>"
              data = data + "<td>" +value.category.category_name+ "</td>"
              data = data + "<td>" +value.templete_sub_category_name+ "</td>"
              data = data + "<td>"
              data = data + "<button class = 'btn btn-primary mr-2 btn-sm'> edit </button>"
              data = data + "<button class = 'btn btn-danger mr-2 btn-sm'> delete </button>"
              data = data + "</td>"
              data = data + "</tr>"
            })
            $('tbody').html(data);
	 ...
	 function addData(){
        ...
        $.ajax({
          ...
          url: "/admin/template/sub-category/store",
