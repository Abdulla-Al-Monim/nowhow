...
class UserContrller extends Controller
{
    ...
    public function updateAccDetailAjax(Request $request){
        ...
        $save_image =1;
        $data = $request->id;
        $data = User::findOrFail($request->id)->update([
            'name'                              => $request->name,
    ...
    public function userSubscrive($id){
        $data = UserSubscrive::insertGetId([
            'user_id'                   => Auth()->User()->id,
            'subscrive_user_id'         => $id,
            'created_at'                => Carbon::now(),
        ]);
        return redirect()->back();
    }
    public function userUnSubscrive($id,$subscribeId){
        $data = UserSubscrive::where('user_id',$id)->where('subscrive_user_id',$subscribeId)->first();
            $userSub = UserSubscrive::Find($data->id);
            $userSub->delete();
        return redirect()->back();
    }
