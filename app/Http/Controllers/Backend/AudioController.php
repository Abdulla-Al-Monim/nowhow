...
class AudioController extends Controller
{
    ...
    public function store(Request $request){
        ...
        $count = Audio::where('slug',$slug)->count();
            ...
            if ($request->image) {
                ...
                $location               = public_path('uploads/audio/thambnail/'.$name_gen);
            ...
            if ($request->image) {
                ...
                $location               = public_path('uploads/audio/images/flexflix_details/'.$name_gen);
                Image::make($image)->resize(750,469)->save($location);
                $image_flexflix_details = 'uploads/audio/images/flexflix_details/'.$name_gen;
            }
            if ($request->image) {
                $image = $request->file('image');
                $name_gen = "flexflix". time() . '.' . $image->extension();
                $location               = public_path('uploads/audio/images/flexflix/'.$name_gen);
                Image::make($image)->resize(270,169)->save($location);
                $image_flexflix = 'uploads/audio/images/flexflix/'.$name_gen;
            }
            if ($request->image) {
                $image = $request->file('image');
                $name_gen = "image_details" .  time() . '.' . $image->extension();
                $location               = public_path('uploads/audio/images/image_details/'.$name_gen);
                Image::make($image)->resize(600,600)->save($location);
                $image_details = 'uploads/audio/images/image_details/'.$name_gen;
            }
            if ($request->image) {
                $image = $request->file('image');
                $name_gen = "image_home" . time() . '.' . $image->extension();
                $location               = public_path('uploads/audio/images/image_home/'.$name_gen);
                Image::make($image)->resize(240,240)->save($location);
                $image_home = 'uploads/audio/images/image_home/'.$name_gen;
            }
            ...
            if ( $request->file ) 
            {
                ...
                $request->file->move(public_path('uploads/audio/file'), $fileName);
                $file             = 'uploads/audio/file/' . $fileName;
    ...
    public function audioDownload($id){
        ...
        $video              = Audio::Find($audio->id);
        $download           = $audio->download + 1;
